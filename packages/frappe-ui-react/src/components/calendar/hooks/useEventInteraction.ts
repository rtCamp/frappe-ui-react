...
const repositionReducer = (state: { xAxis: number; yAxis: number }, action: { type: string; payload?: { x: number; y: number } }) => {
  switch (action.type) {
    case 'MOVE':
      if (!action.payload) {
        return state;
...
export const useEventInteraction = (event: CalendarEvent, config: CalendarConfig) => {
  const [updatedEvent, setUpdatedEvent] = useState(event);
  const [isPopoverOpen, setPopoverOpen] = useState(false);
  const [isEditModalOpen, setEditModalOpen] = useState(false);
  const [isResizing, setIsResizing] = useState(false);
  const [isRepositioning, setIsRepositioning] = useState(false);
  const [repositionState, dispatchReposition] = useReducer(repositionReducer, { xAxis: 0, yAxis: 0 });
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (_event: MouseEvent) => {
      ...
      if (refs.floating.current && !refs.floating.current.contains(_event.target as Node) &&
        eventRef.current && !eventRef.current.contains(_event.target as Node)) {
  ...
  const handleEventClick = useCallback((_event: React.MouseEvent<HTMLDivElement>) => {
  ...
  const handleResizeMouseDown = useCallback((_event: React.MouseEvent<HTMLDivElement>) => {
    if (!eventRef.current) {
      return;
    }
    ...
    const resize = (moveEvent: MouseEvent) => {
      ...
      const snappedHeight = Math.round(newHeight / height_15_min) * height_15_min;

      eventRef.current.style.height = `${snappedHeight}px`;

      const newEndTimeMinutes = calculateMinutes(event.from_time) + (snappedHeight / minuteHeight);
