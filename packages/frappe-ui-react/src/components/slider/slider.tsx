...
const Slider = ({
  ...
  const calculatePercent = useCallback(
    ...
    [min, max, thumbWidth, trackWidth]
  );

  const minPercent = useMemo(
    () => calculatePercent(minVal),
    [minVal, calculatePercent]
  );

  const maxPercent = useMemo(
    () => calculatePercent(maxVal),
    [maxVal, calculatePercent]
  );

  // Measure thumb width and track width.
  useLayoutEffect(() => {
    if (!tooltipHelperRef.current || !trackRef.current) return;

    const thumbElement = tooltipHelperRef.current;
    const trackElement = trackRef.current;

    const observer = new ResizeObserver(() => {
      const newThumbWidth = knob ? thumbElement.offsetWidth : 0;
      const newTrackWidth = trackElement.offsetWidth;
      ...
      <div className={clsx("w-full relative", trackHeightClasses)}>
        <div className="w-full h-full absolute flex items-center justify-between">
          ...
          >
            <div
              className="absolute pointer-events-none -translate-x-1/2"
              style={{
                left: `${range ? maxPercent : minPercent}%`,
              }}
            >
              <div
        ...
        <div ref={trackRef} className="w-full relative">
          ...
          <div
            ...
            style={!range ? { width: `${minPercent}%` } : undefined}
          />
        </div>
      </div>
      {showValue && (
