...
describe("Autocomplete Component", () => {
  ...
  describe("1. Basic Rendering & Initial State", () => {
    ...
    test("BR-03: Renders with pre-selected values (multiple)", () => {
      render(
        <Autocomplete options={simpleOptions} value={["a", "c"]} multiple />
      );
    ...
    test("UI-01 & UI-05: Toggles popover on button click and closes on outside click", async () => {
      render(
        <div>
          ...
          <Autocomplete options={simpleOptions} value={null} />
        </div>
      );
  ...
  describe("3. Single-Select Mode (multiple={false})", () => {
    ...
    test("SS-02: Changes a selection", async () => {
      const onChange = jest.fn();
      render(
        <Autocomplete options={simpleOptions} onChange={onChange} value="a" />
      );
      await userEvent.click(
        screen.getByRole("button", { name: "Toggle options" })
      );
  ...
  describe("5. Search & Filtering", () => {
    test("SF-01, SF-02, SF-03: Filters options by label/value (case-insensitive)", async () => {
      render(
        <Autocomplete
          options={[{ label: "Apple Pie", value: "fruit_1" }]}
          value={null}
        />
      );
      await userEvent.click(
  ...
  describe("7. Advanced Props & Edge Cases", () => {
    ...
    test("AP-02: Disabled options are not selectable", async () => {
      ...
      expect(disabledOption.parentElement?.parentElement).toHaveAttribute(
        "aria-disabled",
        "true"
      );
    ...
    test("AP-03: `maxOptions` limits visible items per group", async () => {
      render(
        <Autocomplete options={simpleOptions} maxOptions={2} value={null} />
      );
      await userEvent.click(
        screen.getByRole("button", { name: "Toggle options" })
      );
    ...
    test("AP-04: `itemPrefix` and `itemSuffix` render content", async () => {
      const itemPrefix = jest.fn(() => (
        <span data-testid="item-prefix">P</span>
      ));
      const itemSuffix = jest.fn(() => (
        <span data-testid="item-suffix">S</span>
      ));
      render(
        <Autocomplete
          options={simpleOptions}
