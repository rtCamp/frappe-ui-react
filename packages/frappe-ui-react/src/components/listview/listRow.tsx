...
const ListRow: React.FC<ListRowProps> = ({
  ...
  const isSelected = useMemo(
    ...
    [list.selections, row, list.rowKey]
  );
  ...
  const roundedClass = useMemo(() => {
    ...
    const rows = list.rows.some((k) => k.group)
      ? list.rows.flatMap((k) => k.rows)
      : list.rows;
    ...
    const atBottom = !selections.includes(
      rows[currentIndex + 1]?.[list.rowKey]
    );
