...
const FontColor = ({ editor, children }: FontColorProps) => {
  ...
  const setBackgroundColor = (color: ColorOption) => {
    if (color.name !== "Default") {
      editor
        .chain()
        .focus()
      ...
      body={({ close }) => (
        <div className="w-max rounded-lg border border-outline-gray-1 bg-surface-modal shadow-xl">
          <div className="p-2">
            ...
            <div className="mt-2 text-sm text-ink-gray-7">Background Color</div>
            <div className="mt-1 grid grid-cols-6 gap-1">
                ...
                <div key={color.name} className="flex">
                  <Tooltip text={color.name}>
                    <button
                      aria-label={color.name}
                      className={clsx(
                        "flex h-5 w-5 items-center justify-center rounded border border-outline-gray-1 text-base",
                        color.class
                      )}
                      onClick={() => {
