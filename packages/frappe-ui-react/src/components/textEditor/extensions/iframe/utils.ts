export interface IframeOptions extends Record<string, unknown> {
...
export function validateURL(url: string, options: IframeOptions): boolean {
  try {
    ...
    if (options.blockedDomains?.some((blocked: string) => domain.includes(blocked))) {
      return false;
    }

    // If allowedDomains is specified, check if domain is allowed
    if (options.allowedDomains?.length) {
      return options.allowedDomains.some((allowed: string) =>
