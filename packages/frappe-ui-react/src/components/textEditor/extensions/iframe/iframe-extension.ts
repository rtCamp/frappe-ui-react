          ...
          handlePaste: (view: any, event: ClipboardEvent) => {
            ...
            if (text) {
              ...
              if (urlPattern.test(text.trim())) {
                return createIframeNode(view, self, text.trim());
              }
            }
...
function createIframeNode(
  ...
  const finalWidth =
    width || getOptimalDimensions(processedUrl, editorWidth).width;
  const finalHeight =
    height || getOptimalDimensions(processedUrl, editorWidth).height;
