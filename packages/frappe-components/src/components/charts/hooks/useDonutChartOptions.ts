...
import type { TooltipComponentOption, SingleAxisComponentOption, PieSeriesOption } from 'echarts';
...
export default function useDonutChartOptions(config: DonutChartConfig) {
  ...
  const orient = 'horizontal' as SingleAxisComponentOption['orient'];
  const bottom = 0;
  const left = 'center';
  const padding = [0, 10, 10, 10];
  ...
  return {
    ...
    series: [
      {
        type: 'pie' as PieSeriesOption['type'],
